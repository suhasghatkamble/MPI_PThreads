...
int main(int argc, char** argv) {
    ...
    my_struct data; //struct object create
    MPI_Datatype struct_type; //new datatype name declare

    // Create the datatype for my_struct
    int blocklengths[3] = {1,1,1};
    MPI_Aint displacements[3]; //displacements array starting address to next
    MPI_Datatype types[3] = {MPI_INT, MPI_DOUBLE, MPI_CHAR};

    displacements[0] = offsetof(my_struct, a); //offsetof gives address
